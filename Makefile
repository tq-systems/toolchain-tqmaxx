...
DISTRO = ubuntu
DISTRO_VERSIONS = 18.04 20.04 22.04
BASE_IMAGES = $(addprefix base-${DISTRO}-,${DISTRO_VERSIONS})
DISTRO_VERSIONS_PREHISTORIC = 14.04 16.04
BASE_IMAGES_PREHISTORIC = $(addprefix base-${DISTRO}-,${DISTRO_VERSIONS_PREHISTORIC})
...
YOCTO_VERSIONS = 18.04 20.04 22.04
...
YOCTO_VERSIONS_PREHISTORIC = 16.04
YOCTO_BASE_IMAGES_PREHISTORIC = $(addprefix yocto-base-${DISTRO}-,${YOCTO_VERSIONS_PREHISTORIC})
YOCTO_DEVEL_IMAGES_PREHISTORIC = $(addprefix yocto-devel-${DISTRO}-,${YOCTO_VERSIONS_PREHISTORIC})
YOCTO_IMAGES_PREHISTORIC = $(addprefix yocto-${DISTRO}-,${YOCTO_VERSIONS_PREHISTORIC})

BARE_VERSIONS = 18.04 22.04 24.04
...
ALL_IMAGES = \
	...
	${CCS_BASE_IMAGES} \
	${PYTHON_IMAGES}
ALL_IMAGES_PREHISTORIC = \
	${BASE_IMAGES_PREHISTORIC} \
	${PTX_BASE_IMAGES} \
	${PTX_DEVEL_IMAGES} \
	${PTX_IMAGES} \
	${YOCTO_BASE_IMAGES_PREHISTORIC} \
	${YOCTO_DEVEL_IMAGES_PREHISTORIC} \
	${YOCTO_IMAGES_PREHISTORIC}

all-prehistoric:
	export REGISTRY_BASE_URL=${REGISTRY_BASE_URL} && docker compose build ${BUILD_ARGS} ${ALL_IMAGES_PREHISTORIC}

new-prehistoric:
	export REGISTRY_BASE_URL=${REGISTRY_BASE_URL} && docker compose build --no-cache --force-rm ${BUILD_ARGS} ${ALL_IMAGES_PREHISTORIC}
